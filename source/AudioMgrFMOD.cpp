	...
	)
{
	...
	//utf8string folder;
	//File::getCurrentFolder(folder);
	//DBOUT("Loading file:" << name << " from path:" << folder);
	...
	)
{
	...
	//ifstream *pF = s_openFiles[(size_t)handle];
	//assert(!pF->bad());
	//pF->close();
	//delete pF;
...
bool AudioMgrFMOD::Initialise(void)
{
	...
	if (m_pSystem->setFileSystem(FileBridge::userOpen, FileBridge::userClose, FileBridge::userRead, FileBridge::userSeek, -1) != FMOD_OK)
		assert(false);
	
	assert( !m_pSongMgr && !m_pSfxMgr );
											...
											unsigned int  commanddata2 )
{
	if( type == FMOD_CHANNEL_CALLBACKTYPE_END )
	{
		...
		if (pCh->getUserData(&pCD) != FMOD_OK)
			assert(false);
		static_cast<AudioGroupFMOD::ChannelData *>(pCD)->_channelHandle = UINT_MAX;
	}
	return FMOD_OK;	
}
...
bool AudioGroupFMOD::Load( const utf8string &folder )
{
	...
	{
		vector<utf8string> names = File::findFiles( folder, fileSpec[i].c_str() );
		if( names.empty() )
			continue;
		...
		{
			SoundData data;
			if(!splitFileName(names[ii],NULL,NULL,NULL,&data._name))
				continue;
			...
			else
			{
				...
				if( res != FMOD_OK ) 
				{ 
					assert(false);// , "FMOD ERROR (%d) %s", res, FMOD_ErrorString(res));
					success = false; 
				}
			}
			if( success )
...
const string &AudioGroupFMOD::GetName( const unsigned int channelHandle )
{
...
{
	assert( idx >= 0 && idx < static_cast<int>(m_channels.size()) );
...
int AudioGroupFMOD::GetSoundData( const utf8string &name, SoundData ** pSD )
{
...
bool AudioGroupFMOD::Exists( const utf8string &name, int *pSoundIndex )
{
	utf8string sname;
	int hdl;
	//if( !CMN::getSubStr( name, "(.*)\\....", sname ) )
	if(!splitFileName(name,NULL,NULL,NULL,&sname))
	{
...
bool AudioGroupFMOD::Play( const utf8string &name, const bool loop, const bool paused, unsigned int *pChannelHandle, const float vol )
{
...
bool AudioGroupFMOD::Play( const unsigned int handle, const bool loop, const bool paused, unsigned int *pChannelHandle, const float vol )
{
	...
	pCh->setMode((loop)?FMOD_LOOP_NORMAL:FMOD_LOOP_OFF);
	pCh->setLoopCount((loop)? -1 : 0 );
